    ...
    for (let r = 0; r < 6; r ++) {
        ...
        for (let c = 0; c < 7; c ++) {
            ...
            if (dayCount) {
                ...
                if (toData) {
                    toData.forEach((data) => {
                        dayBox.appendChild(createLabel(data.title, data.color));
                    });
                }
            }
...
const color_s = [ "black", "gray", "green", "skyblue", "blue", "purple", "yellow", "orange", "red" ];

function addHomeWork(title, about, created_at, color)
{
    ...
    for (let i = 0; i < color_s.length; i ++) {
        ...
        option.innerText = color_s[i];
        option.onclick = () => {
            colorSelector.style.borderColor = color_s[i];
        }
    ...
    button.onclick = () => {
        if (input.value.trim() !== "" && calender.value !== "" && textarea.value.trim() !== "") {
            ...
            saveHomeWork(input.value.trim(), textarea.value.trim(), date_s[0], date_s[1], date_s[2], title, colorSelector.value);
        } else {
    ...
    main.appendChild(colorSelector);
...
function saveHomeWork(title, about, year, month, date, edit, color) {
    let data_s = [];
...
function createHomeWorkList()
{
    if (localStorage.length !== null) {
        ...
        data_s.forEach((data) => {
            ...
            list.style.borderColor = data.color;
            list.onclick = () => {
            ...
            edit_img.alt = "homework to edit";
            edit_img.onclick = () => {
                addHomeWork(data.title, data.about, data.created_at, data.color);
            }

            const delete_img = document.createElement("img");
            delete_img.src = "img_s/delete.png";
            delete_img.alt = "homework to delete";
            delete_img.onclick = () => {
...
function createManagerData()
{
    ...
    if (toData) {
        toData.forEach((data) => {
            toDayHomeWork.appendChild(createHomeWorkLabel(data.title, data.about, data.color));
        });
    }
    ...
    if (checkData_s.length) {
        const checkData = checkData_s.sort((a, b) => { return (a < b) ? -1 : 1 });
        let is_first = true;
        checkData.forEach((data) => {
            if (data > parseInt(formatDateToInputValue(toDate.getFullYear(), toDate.getMonth() + 1, toDate.getDate()).split("-").join("")) && is_first) {
                const checkDataKey = data.toString().slice(0, 4) + "-" + data.toString().slice(4, 6) + "-" + data.toString().slice(6, 8);
                JSON.parse(localStorage.getItem(checkDataKey)).forEach((data) => {
                    nearDayHomeWork.appendChild(createHomeWorkLabel(data.title, data.about, data.color));
                });
                is_first = false;
            }
        });
...
function createHomeWorkLabel(title, about, color)
{
